...
def configure_incel():
  ...
  assert xai_api_key, "Please set XAI_API_KEY environment variable"
  logging.getLogger("xai").setLevel(logging.WARNING)
  print(f"Configured xAI with XAI_API_KEY={xai_api_key}")
...
def get_incel_response(xai_client,
                       callback,
                       personae = None,
                       model_name: str = default_llm("xai"),
                       prompt: str = "",
                       timestamp: int = 0,
  ...
  try:
    response_obj = xai_client.chat.completions.create(
      messages=messages, model=default_llm("xai"))
    # print(response_obj)
...
def test_incel():
  ...
  xai_client = configure_openai()
  result = get_incel_response(xai_client, "What is your name?", 0, _callback)
  print(result)
